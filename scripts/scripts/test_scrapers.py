...
def test_hackerone():
    ...
    if not token:
        ...
        print("   Set it with: export HACKERONE_TOKEN='your_token'")
        return False
    ...
    try:
        ...
        print(f"  - Has title: {'✓' if 'title' in report else '✗'}")
        print(f"  - Has description: {'✓' if 'description' in report else '✗'}")
        print(f"  - Has vulnerability_type: {'✓' if 'vulnerability_type' in report else '✗'}")
        print(f"  - Has severity: {'✓' if 'severity' in report else '✗'}")
        ...
        print(f"  Bounty: ${report.get('bounty', 0)}")
...
def test_cve():
    ...
    try:
        ...
        if cves:
            cve = cves[0]
            print(f"\n📊 Sample CVE:")
            print(f"  ID: {cve.get('cve_id', 'N/A')}")
            print(f"  Description: {cve.get('description', 'N/A')[:80]}...")
...
def main():
    ...
    print("  export HACKERONE_TOKEN='your_token'")
    print("  export BUGCROWD_TOKEN='your_token'  # optional")
    print("  export NVD_API_KEY='your_key'       # optional")
